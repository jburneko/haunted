...
export class SocketHandler {
  static processEvent(data) {
    switch (data.event_id) {
      case `SOCKETEVENT.refreshToken`:
        SocketHandler.refreshToken(data.data);
        break;
      case `SOCKETEVENT.decreasePresence`:
        SocketHandler.decreasePresence(data.data);
        break;
      case `SOCKETEVENT.spendHelpDice`:
        SocketHandler.spendHelpDice(data.data);
        break;
      case `SOCKETEVENT.addConflictRoll`:
        SocketHandler.addConflictRoll(data.data);
        break;
    }
  }
  ...
  static spendHelpDice(helpers) {
    if (UserUtils.isSourceOfTruth()) HauntedActor._spendHelpDice(helpers);
  }
